...
*/
    ...
    var Packer = Base.extend({
    	...
    	pack2: function(script, callback) {
    		script = this.minify(script + "\n");
    	...
    	pack: function(script, base62, shrink, minimal) {
    		var p = new Packer();
    		var packed = p.pack(script, base62, shrink);
	    	
	    	if(minimal) {
    			var index = packed.indexOf(HEADER)+HEADER.length;
    			packed = packed.substring(index, packed.length - SUFFIX.length - 1);
    		}
    		return packed;
    	},
    	unpack: function(script) {
    		return eval(js.sf("(%s(%s%s)", Packer.header.toString(), script, SUFFIX));
    	}
    	
    });
    
    Packer.header = function (p, a, c, k, e, r) {
	    e = function (c) {
