define(function(require) {
	...
	Instance = Instance(require, {
		...
		prototype: {
			 ...
			 * @param object
			 */
			constructor: function(entity, key, object) {
				this._entity = entity;
				this._key = key;
				this._object = object || {};
				this._dirty = {};
...
//				Persistence.instanceNotifyEvent(this, event, data);
	...
	//console.log(String.format("%n.%s := %n", this, name, value));
				...
				if(name.length > 1) {
					...
					if(oldValue === undefined || changes === undefined || this._dirty[name] === undefined) {
						...
						if(changed === true) {
							...
							if(/*attribute.type !== 'one-to-many' && */(hadValue === true || isNew === true)) {
			...
			setAttributeValues: function(values, prefix) {
				...
				for(var k in values) {
					...
					if(v !== null && typeof v === "object" && !(v instanceof Date) &&
							!(v instanceof Instance) && !(v instanceof Array)) {
			 ...
			 */
			getKey: function() {
				return this._key !== null ? this._key : String.format("/%d", this._preferredKey || this.hashCode());
			},
			...
			getObject: function() {
				var r = this._obj;
				if(r === null) {
					var Obj = this._entity.getObject();
			 ...
			 */
			getName: function() {
				return this._key;//"";//this._entity.nameOf(this);
			},
			...
			getDirtyObject: function() {
				...
				for(var k in dirty) {
					...
					if(dirty[k].attribute === undefined || (dirty[k].attribute.type !== "one-to-many" &&
							dirty.attribute.type !== "many-to-many")) {
	...
	//console.log(String.format("%n - reset dirty", this), js.cs());
		...
		NameOf: function(instance) {
			...
			if(name !== null && name !== instance._key) {
